...
delete:
	...
	[ -f ${PYTHONPATH}/poco-*.tar.gz ] && rm ${PYTHONPATH}/poco-*.tar.gz && echo "	build artifact - removed" || echo "	OK: no dist tar"
...
#	installs locally
install: delete
	sudo apt-get install -y python3 gir1.2-gtk-3.0 gir1.2-wnck-3.0 gir1.2-appindicator3-0.1 libwnck-3-0 python3-gi-cairo python3-xdg python3-dbus python3-setproctitle python3-xlib libx11-dev 1>/dev/null
	sudo apt-get install -y python3-distutils 1>/dev/null
	echo "	OK: poco dependencies"
	sudo ${SETUP_SCRIPT} install --record $(PYTHONPATH)/installed_files.txt 1>/dev/null
	echo "	OK: poco files installed"
	sudo update-icon-caches /usr/share/icons/* 1>/dev/null && echo "	OK: icons cache updated" || echo "	WARN: failed to update icons cache"
	echo "	SUCCESS: poco installed"
uninstall:
...
sources: delete
	...
	&& echo "$$CHANGES" >> deb_dist/poco_${VERSION}-1_source.changes
...
publish:
	...
	debsign -pgpg2 ${PYTHONPATH}/deb_dist/poco_${VERSION}-1_source.changes && echo "Signed"
	cd ${PYTHONPATH}/deb_dist && dput ppa:pedrosans/poco poco_${VERSION}-1_source.changes && echo "Published"
