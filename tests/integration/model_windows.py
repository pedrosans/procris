...
class ServiceIntegrationTestCase(unittest.TestCase):
	...
	def test_read_active(self):
		self.assertEqual(WINDOW_NAME_ONE, windows.active.get_wnck_window().get_name())

	def test_minimize(self):
		first_active_xid = windows.active.get_wnck_window().get_xid()
		...
		self.assertNotEqual(windows.active.xid, first_active_xid)

		run_on_main_loop_and_wait(windows.read_default_screen)
		self.assertNotEqual(WINDOW_NAME_ONE, windows.active.get_wnck_window().get_name())
		self.assertTrue(get_window(WINDOW_NAME_ONE).is_minimized())

	def test_only(self):
		GLib.idle_add(windows.active.only, None, priority=GLib.PRIORITY_HIGH)
		time.sleep(1)
		Popen(['wmctrl', '-a', 'test'])
		self.assertEqual(WINDOW_NAME_ONE, windows.active.get_wnck_window().get_name())
