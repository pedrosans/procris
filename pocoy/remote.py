...
class ForeignInterface (ExportedGObject):
	...
	@dbus.service.method("io.github.pocoy.Service", in_signature='', out_signature='s')
	def get_id(self):
		return str(os.getpid())

	@dbus.service.method("io.github.pocoy.Service", in_signature='s', out_signature='')
	def message(self, ipc_message):
		self.ipc_handler(ipc_message)

	@dbus.service.method("io.github.pocoy.Service", in_signature='', out_signature='')
	def stop(self):
		self.stop()
...
class Proxy:
	...
	def send_message(self, ipc_message):
		self.dbus_proxy.get_dbus_method(
			'message', 'io.github.pocoy.Service'
		)(ipc_message)

	def get_running_instance_id(self):
		get_remote_id = self.dbus_proxy.get_dbus_method('get_id', 'io.github.pocoy.Service')
		return get_remote_id()

	def stop_running_instance(self):
		quit_function = self.dbus_proxy.get_dbus_method('stop', 'io.github.pocoy.Service')
		quit_function()
...
def release():
	...
	print('pocoy service were released from bus')
...
SERVICE_NAME = "io.github.pocoy"
SERVICE_OBJECT_PATH = "/io/github/pocoy"
